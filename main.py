...
recommendations = {
    ...
    "ISFJ": ("🏞️ 잘츠부르크, 오스트리아", "잔잔한 호수와 클래식 음악 속 힐링 여행"),
    "ESTJ": ("📈 도쿄, 일본", "효율적 일정과 깔끔한 시스템 여행"),
    "ESFJ": ("🏖️ 괌", "따뜻한 햇살 아래 가족과 함께하는 휴양 여행"),
    "ISTP": ("🏜️ 요세미티 국립공원, 미국", "대자연 속 자유로운 트레킹 모험"),
    "ISFP": ("🌸 교토 외곽, 일본", "감성적 사진 여행과 조용한 정원 산책"),
    "ESTP": ("🎢 두바이", "럭셔리와 액티비티로 가득한 역동적인 여행"),
    "ESFP": ("🎉 방콕, 태국", "흥겨운 거리와 다채로운 밤문화 체험")
}

# ✅ 추천 결과 출력
if user_mbti:
    with st.spinner("당신에게 딱 맞는 여행지를 찾는 중입니다... 🧠"):
        st.success(f"{user_mbti} 유형에게 추천하는 여행지는...")
        location, description = recommendations.get(user_mbti, ("🌍 지구 어딘가", "모험이 당신을 기다리고 있어요!"))
